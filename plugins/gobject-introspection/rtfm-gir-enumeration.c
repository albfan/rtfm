                             ...
                             GError **error)
{
  ...
  g_clear_pointer (&self->glib_error_domain, g_free);
  ...
  if (!rtfm_gir_g_markup_collect_attributes (element_name, attribute_names, attribute_values, error,
                                             ...
                                             G_MARKUP_COLLECT_STRDUP | G_MARKUP_COLLECT_OPTIONAL, "glib:error-domain", &self->glib_error_domain,
                                   ...
                                   GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_GLIB_ERROR_DOMAIN:
      g_free (self->glib_error_domain);
      self->glib_error_domain = g_value_dup_string (value);
      break;
...
RtfmGirEnumeration *
rtfm_gir_enumeration_new (void)
{
  return g_object_new (RTFM_GIR_TYPE_ENUMERATION, NULL);
}
