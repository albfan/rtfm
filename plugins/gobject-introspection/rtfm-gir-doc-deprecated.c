...
#define G_LOG_DOMAIN "rtfm-gir-doc-deprecated"

#include "rtfm-gir-doc-deprecated.h"

struct _RtfmGirDocDeprecated
{
...
static void
rtfm_gir_doc_deprecated_text (GMarkupParseContext *context,
                              const gchar *text,
                              gsize text_len,
                              gpointer user_data,
                              GError **error)
{
  RtfmGirDocDeprecated *self = user_data;
...
static const GMarkupParser markup_parser = {
  NULL,
  NULL,
  rtfm_gir_doc_deprecated_text,
  NULL,
  NULL,
};

static gboolean
rtfm_gir_doc_deprecated_ingest (RtfmGirParserObject *object,
...
static void
rtfm_gir_doc_deprecated_printf (RtfmGirParserObject *object,
                                GString *str,
                                guint depth)
{
  RtfmGirDocDeprecated *self = (RtfmGirDocDeprecated *)object;
  ...
  for (i = 0; i < depth; i++)
    g_string_append (str, "  ");
  g_string_append (str, "<doc-deprecated");
  ...
  if (self->text != NULL && self->text->len)
    {
      ...
      g_string_append_printf (str, ">%s</doc-deprecated>\n", escaped);
    }
  else
    g_string_append (str, "></doc-deprecated>\n");
}
...
static void
rtfm_gir_doc_deprecated_get_property (GObject    *object,
                                      guint       prop_id,
                                      GValue     *value,
                                      GParamSpec *pspec)
{
...
static void
rtfm_gir_doc_deprecated_set_property (GObject      *object,
                                      guint         prop_id,
                                      const GValue *value,
                                      GParamSpec   *pspec)
{
...
static void
rtfm_gir_doc_deprecated_finalize (GObject *object)
{
  RtfmGirDocDeprecated *self = (RtfmGirDocDeprecated *)object;
...
static void
rtfm_gir_doc_deprecated_class_init (RtfmGirDocDeprecatedClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  RtfmGirParserObjectClass *parent_class = RTFM_GIR_PARSER_OBJECT_CLASS (klass);

  object_class->get_property = rtfm_gir_doc_deprecated_get_property;
  object_class->set_property = rtfm_gir_doc_deprecated_set_property;
  object_class->finalize = rtfm_gir_doc_deprecated_finalize;

  parent_class->ingest = rtfm_gir_doc_deprecated_ingest;
  parent_class->printf = rtfm_gir_doc_deprecated_printf;
...
static void
rtfm_gir_doc_deprecated_init (RtfmGirDocDeprecated *self)
{
}

const gchar *
rtfm_gir_doc_deprecated_get_xml_space (RtfmGirDocDeprecated *self)
{
  g_return_val_if_fail (RTFM_GIR_IS_DOC_DEPRECATED (self), NULL);

  return self->xml_space;
}

const gchar *
rtfm_gir_doc_deprecated_get_xml_whitespace (RtfmGirDocDeprecated *self)
{
  g_return_val_if_fail (RTFM_GIR_IS_DOC_DEPRECATED (self), NULL);
