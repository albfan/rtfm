                             ...
                             gpointer      user_data)
{
  ...
  r = fuzzy_index_builder_write_finish (builder, result, error);
  g_assert (r == TRUE);
...
test_index_builder_basic (void)
{
  ...
  fuzzy_index_builder_insert (builder, "foo", g_variant_new_int32 (1));
  fuzzy_index_builder_insert (builder, "FOO", g_variant_new_int32 (2));
  fuzzy_index_builder_insert (builder, "Foo", g_variant_new_int32 (3));
  fuzzy_index_builder_insert (builder, "bar", g_variant_new_int32 (4));
  fuzzy_index_builder_insert (builder, "baz", g_variant_new_int32 (5));
                           ...
                           gpointer      user_data)
{
  ...
  matches = fuzzy_index_query_finish (index, result, &error);
  g_assert_no_error (error);
  g_assert (matches != NULL);

  n_items = g_list_model_get_n_items (matches);
  g_assert_cmpint (n_items, ==, 5);
  g_assert (FUZZY_TYPE_INDEX_MATCH == g_list_model_get_item_type (matches));

  for (i = 0; i < n_items; i++)
    {
      g_autoptr(FuzzyIndexMatch) match = g_list_model_get_item (matches, i);
      const gchar *key;
      GVariant *doc;
      gfloat score;

      g_assert (FUZZY_IS_INDEX_MATCH (match));

      key = fuzzy_index_match_get_key (match);
      doc = fuzzy_index_match_get_document (match);
      score = fuzzy_index_match_get_score (match);
                          ...
                          gpointer      user_data)
{
  ...
  r = fuzzy_index_load_file_finish (index, result, &error);
  g_assert_no_error (error);
                     ...
                     gpointer      user_data)
{
  ...
  index = fuzzy_index_new ();

  fuzzy_index_load_file_async (index,
                               file,
                               NULL,
      ...
      gchar *argv[])
{
  ...
  g_test_add_func ("/Fuzzy/IndexBuilder/basic", test_index_builder_basic);
  g_test_add_func ("/Fuzzy/Index/basic", test_index_basic);
  return g_test_run ();
}
