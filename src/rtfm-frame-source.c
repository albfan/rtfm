...
static gboolean
rtfm_frame_source_prepare (GSource *source,
                          gint    *timeout_)
{
   RtfmFrameSource *fsource = (RtfmFrameSource *)(gpointer)source;
   gint64 current_time;
   guint elapsed_time;
   guint new_frame_num;
   guint frame_time;
...
static gboolean
rtfm_frame_source_check (GSource *source)
{
   gint timeout_;
   return rtfm_frame_source_prepare(source, &timeout_);
}

static gboolean
rtfm_frame_source_dispatch (GSource     *source,
...
static GSourceFuncs source_funcs = {
   rtfm_frame_source_prepare,
   rtfm_frame_source_check,
   rtfm_frame_source_dispatch,
};

/**
 * rtfm_frame_source_add:
 ...
 */
guint
rtfm_frame_source_add (guint       frames_per_sec,
                      GSourceFunc callback,
                      gpointer    user_data)
{
   RtfmFrameSource *fsource;
   GSource *source;
   guint ret;
   ...
   source = g_source_new(&source_funcs, sizeof(RtfmFrameSource));
   fsource = (RtfmFrameSource *)(gpointer)source;
   fsource->fps = frames_per_sec;
   fsource->frame_count = 0;
   fsource->start_time = g_get_monotonic_time() / 1000;
   g_source_set_callback(source, callback, user_data, NULL);
   g_source_set_name(source, "RtfmFrameSource");
