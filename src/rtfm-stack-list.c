...
#define G_LOG_DOMAIN "rtfm-stack-list"
...
typedef struct
{
  ...
  RtfmRect          *animating_rect;
} RtfmStackListPrivate;
...
G_DEFINE_TYPE_WITH_PRIVATE (RtfmStackList, rtfm_stack_list, GTK_TYPE_BIN)
...
static void
rtfm_stack_list_content_row_activated (RtfmStackList *self,
                                       GtkListBoxRow *row,
                                       GtkListBox    *box)
{
  RtfmStackListPrivate *priv = rtfm_stack_list_get_instance_private (self);

  g_return_if_fail (RTFM_IS_STACK_LIST (self));
...
static void
rtfm_stack_list_header_row_activated (RtfmStackList *self,
                                      GtkListBoxRow *row,
                                      GtkListBox    *box)
{
  RtfmStackListPrivate *priv = rtfm_stack_list_get_instance_private (self);
...
static gboolean
rtfm_stack_list__overlay__get_child_position (RtfmStackList *self,
                                              GtkWidget     *widget,
                                              GdkRectangle  *rect,
                                              GtkOverlay    *overlay)
{
  ...
  rtfm_rect_get_rect (priv->animating_rect, rect);
...
static void
rtfm_stack_list_scroll_to_top (RtfmStackList *self)
{
  RtfmStackListPrivate *priv = rtfm_stack_list_get_instance_private (self);
  GtkAdjustment *vadj;
...
static void
rtfm_stack_list_end_anim (RtfmStackList *self)
{
  RtfmStackListPrivate *priv = rtfm_stack_list_get_instance_private (self);
  ...
  if (priv->animation != NULL)
    {
      rtfm_animation_stop (priv->animation);
...
{
  RtfmStackListPrivate *priv;
  RtfmStackList *self;
  GtkListBoxRow *row;
  gpointer *closure = data;

  g_assert (closure != NULL);
  g_assert (RTFM_IS_STACK_LIST (closure [0]));
  g_assert (GTK_IS_LIST_BOX_ROW (closure [1]));

  self = closure [0];
  row = closure [1];

  priv = rtfm_stack_list_get_instance_private (self);

  if (row == priv->animating)
    rtfm_stack_list_end_anim (self);
...
static void
rtfm_stack_list_begin_anim (RtfmStackList      *self,
                            GtkListBoxRow      *row,
                            ...
                            const GdkRectangle *end_area)
{
  ...
  priv->animation = rtfm_object_animate_full (priv->animating_rect,
...
static void
rtfm_stack_list_real_header_activated (RtfmStackList *self,
                                       GtkListBoxRow *header)
{
  gint pos;
  ...
  while (rtfm_stack_list_get_depth (self) > (guint)pos)
    rtfm_stack_list_pop (self);
}

static void
rtfm_stack_list_finalize (GObject *object)
{
  RtfmStackList *self = (RtfmStackList *)object;
  RtfmStackListPrivate *priv = rtfm_stack_list_get_instance_private (self);
  ...
  G_OBJECT_CLASS (rtfm_stack_list_parent_class)->finalize (object);
}

static void
rtfm_stack_list_get_property (GObject    *object,
                              guint       prop_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
  RtfmStackList *self = RTFM_STACK_LIST (object);

  switch (prop_id)
    {
    case PROP_MODEL:
      g_value_set_object (value, rtfm_stack_list_get_model (self));
      break;
...
static void
rtfm_stack_list_class_init (RtfmStackListClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->finalize = rtfm_stack_list_finalize;
  object_class->get_property = rtfm_stack_list_get_property;

  klass->header_activated = rtfm_stack_list_real_header_activated;
  ...
  signals [HEADER_ACTIVATED] =
    g_signal_new ("header-activated",
                  ...
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (RtfmStackListClass, header_activated),
                  NULL, NULL, NULL,
                  G_TYPE_NONE,
                  1,
  ...
  signals [ROW_ACTIVATED] =
    g_signal_new ("row-activated",
                  G_TYPE_FROM_CLASS (klass),
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (RtfmStackListClass, row_activated),
                  NULL, NULL, NULL,
                  G_TYPE_NONE,
                  1,
                  GTK_TYPE_LIST_BOX_ROW);
...
static void
rtfm_stack_list_init (RtfmStackList *self)
{
  RtfmStackListPrivate *priv = rtfm_stack_list_get_instance_private (self);

  priv->animating_rect = g_object_new (RTFM_TYPE_RECT, NULL);
  ...
  g_signal_connect_object (priv->overlay,
                           "get-child-position",
                           G_CALLBACK (rtfm_stack_list__overlay__get_child_position),
                           self,
                           G_CONNECT_SWAPPED);
  ...
  g_signal_connect_object (priv->headers,
                           "row-activated",
                           G_CALLBACK (rtfm_stack_list_header_row_activated),
                           self,
                           G_CONNECT_SWAPPED);
...
GtkWidget *
rtfm_stack_list_new (void)
{
  return g_object_new (RTFM_TYPE_STACK_LIST, NULL);
}

void
rtfm_stack_list_push (RtfmStackList                 *self,
                      GtkWidget                     *header,
                      GListModel                    *model,
                      RtfmStackListCreateWidgetFunc  create_widget_func,
                      gpointer                       user_data,
                      GDestroyNotify                 user_data_free_func)
{
  ...
  rtfm_stack_list_begin_anim (self, GTK_LIST_BOX_ROW (header), &current_area, &target_area);
}

void
rtfm_stack_list_pop (RtfmStackList *self)
{
  RtfmStackListPrivate *priv = rtfm_stack_list_get_instance_private (self);
  ModelInfo *info;
...
/**
 * rtfm_stack_list_get_model:
 *
 * Returns: (transfer none): An #RtfmStackList.
 */
GListModel *
rtfm_stack_list_get_model (RtfmStackList *self)
{
  RtfmStackListPrivate *priv = rtfm_stack_list_get_instance_private (self);
  ModelInfo *info;

  g_return_val_if_fail (RTFM_IS_STACK_LIST (self), NULL);
...
guint
rtfm_stack_list_get_depth (RtfmStackList *self)
{
  RtfmStackListPrivate *priv = rtfm_stack_list_get_instance_private (self);

  g_return_val_if_fail (RTFM_IS_STACK_LIST (self), 0);

  return priv->models->len;
}

void
rtfm_stack_list_clear (RtfmStackList *self)
{
  RtfmStackListPrivate *priv = rtfm_stack_list_get_instance_private (self);
